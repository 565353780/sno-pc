...
/* Subroutine */ int dicfs_(size_t *n, size_t *nnz, double *a, double *adiag,
                            size_t *acol_ptr__, size_t *arow_ind__, double *l,
                            double *ldiag, size_t *lcol_ptr__,
                            size_t *lrow_ind__, size_t *p, double *alpha,
...
#pragma omp parallel for private(i__)
#endif
      for (i__ = acol_ptr__[j]; i__ <= (ptrdiff_t)(acol_ptr__[j + 1] - 1);
           ++i__) {
        l[i__] = a[i__] * wa2[j] * wa2[arow_ind__[i__]];
      }
    }
    /*        Attempt an incomplete factorization. */
    dicf_(n, nnz, &l[1], &ldiag[1], &lcol_ptr__[1], &lrow_ind__[1], p, &info,
          &iwa[1], &iwa[*n + 1], &iwa[(*n << 1) + 1], &wa1[1]);
    /*        If the factorization exists, then test for
     * terstd::min<double>ation. */
...
int dsel2_(size_t *n, double *x, size_t *keys, size_t *k) {
  ...
  while (l < u) {
    ...
    /*          a(l) <= a(p1), a(p2) <= a(p3), std::max<double>(a(p1),a(p3)) <=
     * a(u). */
