...
void HLBFGS::initialize_setting() {
  ...
  info[9] =
      0; // 0: linesearch 1: modified linesearch (do not set 1 in practice !)
  info[10] = 0; // 0: Disable preconditioned CG 1: preconditioned CG
  info[11] = 1; // different methods for choosing beta in CG.
  info[12] =
      1; // internal usage. 0: only update diag in update_q_by_inverse_hessian
  info[13] = 0; // 0: standard lbfgs update, 1: Biggs's update, 2: Yuan's
                // update; 3: Zhang and Xu's update
  info[14] = 0; // 0: standard Augmented Lagrangian method; 1:
                // Powell-Hestenes-Rockafellar Augmented Lagrangian method
  info[15] = 0; // 1: dynamically adjust sigma in AL-method for each constraints
                // (experimental)
  info[16] = 0;
}
//////////////////////////////////////////////////////////////////////////
void HLBFGS::print_message(bool print, size_t id) {
  ...
  case 5:
    ...
    break;
  case 6:
    std::cout << "Convergence: max element of (x-x_prev) <= " << parameters[11]
              << std::endl;
    break;
  case 7:
...
#pragma omp parallel for
    ...
    for (ptrdiff_t i = 0; i < (ptrdiff_t)n_N_; i++) {
      diag[i] = 1.0 / (dyy / (ys * diag[i]) + y_start[i] * y_start[i] / ys -
                       dyy * s_start[i] * s_start[i] /
                           (ys * dinvss * diag[i] * diag[i]));
    }
    if (info[12] == 1) {
#ifdef USE_OPENMP
#pragma omp parallel for
...
void HLBFGS::update_first_step(double *q, double *s, double *y, double *rho,
                               double *alpha, size_t bound, size_t curpos,
                               size_t iter) {
  size_t start;
  double tmp;
  ...
  for (size_t i = bound; i >= 0; i--) {
    start =
        iter <= n_M_ ? curpos + i - bound : (curpos + i + n_M_ - bound) % n_M_;
...
void HLBFGS::conjugate_gradient_update(double *q, double *prev_q_update,
  ...
  if (info[11] == 1) {
    if (info[2] == 0) {
      ...
      cg_beta /= std::fabs(cg_beta - HLBFGS_DDOT(n_N_, q, prev_q_first_stage));
      memcpy(prev_q_first_stage, q, sizeof(double) * n_N_);
    }
  } else {
    if (info[2] == 0) {
...
void HLBFGS::build_hessian_info() {
  ...
  l_info.set_l_size(m_hessian.get_nonzeros() +
                    m_hessian.get_dimension() * l_info.get_p());
...
//////////////////////////////////////////////////////////////////////////
void HLBFGS::optimize_without_constraints(double *init_sol, size_t max_iter,
                                          void *user_pointer, int *it) {
  if (self_check() == false) {
  ...
  do {
    if (info[7] == 1 && ((T == 0) || (info[2] % T == 0))) {
      (*evalhessian_callback)(n_N_, variables_, f, gradient_, m_hessian,
                              user_pointer);
      build_hessian_info();
    } else if (info[2] == 0) {
...
#pragma omp parallel for
      ...
      if (info[13] == 1) {
        update_alpha =
            1.0 /
            (rho[cur_pos] * 6 * (prev_f - f + HLBFGS_DDOT(n_N_, g, &s[start])) -
             2.0);
      } else if (info[13] == 2) {
        update_alpha = 1.0 / (rho[cur_pos] * 2 *
                              (prev_f - f + HLBFGS_DDOT(n_N_, g, &s[start])));
      } else if (info[13] == 3) {
        update_alpha =
            1.0 /
            (1 + rho[cur_pos] * (6 * (prev_f - f) +
                                 3 * (HLBFGS_DDOT(n_N_, g, &s[start]) +
                                      HLBFGS_DDOT(n_N_, prev_g, &s[start]))));
      }
      if (info[13] != 0) {
...
#pragma omp parallel for
    ...
    do {
      MCSRCH(&n_N_, x, &f, g, q, &stp, &ftol, &gtol, &xtol, &stpmin, &stpmax,
             &maxfev, &linesearch_info, &nfev, wa, keep, rkeep,
      ...
      if (info[9] == 1 &&
          prev_f > f) // modify line search to avoid too many function calls
      {
        linesearch_info = 1;
        break;
      }
...
//////////////////////////////////////////////////////////////////////////
void HLBFGS::optimize_with_constraints(double *init_sol, size_t max_iter,
                                       size_t local_iter, void *user_pointer,
                                       bool warm_start, double *init_lambda,
                                       double *init_sigma) {
  if (self_check() == false || n_E_ + n_IE_ == 0 ||
      eval_constraints_callback == 0) {
    std::cerr << "please check your input!" << std::endl;
    return;
  }
  ...
  d.resize(n_E_ + n_IE_);
  sigma = parameters[10];
  if (warm_start && init_sigma)
    sigma = std::max(parameters[10], *init_sigma);
...
#pragma omp parallel for
          ...
          for (ptrdiff_t i = 0; i < (ptrdiff_t)(n_E_ + n_IE_); i++) {
            lambda[i] = std::min(std::max(min_lambda, lambda[i] - sigma * d[i]),
                                 max_lambda);
          }
        } else {
#ifdef USE_OPENMP
#pragma omp parallel for
          ...
          for (ptrdiff_t i = 0; i < (ptrdiff_t)n_E_; i++) {
            lambda[i] = std::min(
                std::max(min_lambda, lambda[i] + sigma * constraint_values_[i]),
                max_lambda);
          }
#ifdef USE_OPENMP
#pragma omp parallel for
#endif
          for (ptrdiff_t i = (ptrdiff_t)n_E_; i < (ptrdiff_t)(n_E_ + n_IE_);
               i++) {
            lambda[i] = std::min(
                std::max(0.0, lambda[i] - sigma * constraint_values_[i]),
                max_mu);
          }
        }
        K = max_d;
...
#pragma omp parallel for
        ...
        for (ptrdiff_t i = 0; i < (ptrdiff_t)(n_E_ + n_IE_); i++) {
          ...
          if (md < parameters[7] * prev_K[i]) {
            lambda[i] = std::min(
                std::max(min_lambda, lambda[i] - dynamic_sigma[i] * d[i]),
                max_lambda);
            prev_K[i] = md;
          } else
...
#pragma omp parallel for
        ...
        for (ptrdiff_t i = 0; i < (ptrdiff_t)n_E_; i++) {
          ...
          if (md < parameters[7] * prev_K[i]) {
            lambda[i] = std::min(
                std::max(min_lambda,
                         lambda[i] + dynamic_sigma[i] * constraint_values_[i]),
                max_lambda);
            prev_K[i] = md;
          } else
            dynamic_sigma[i] *= parameters[8];
...
#pragma omp parallel for
        ...
        for (ptrdiff_t i = (ptrdiff_t)n_E_; i < (ptrdiff_t)(n_E_ + n_IE_);
             i++) {
          const double md = std::fabs(
              std::max(-constraint_values_[i], -lambda[i] / dynamic_sigma[i]));
          if (md < parameters[7] * prev_K[i]) {
            lambda[i] =
                std::min(std::max(0.0, lambda[i] - dynamic_sigma[i] *
                                                       constraint_values_[i]),
                         max_mu);
            prev_K[i] = md;
          } else
            dynamic_sigma[i] *= parameters[8];
    ...
    if (newiteration_constraints_callback) {
      newiteration_constraints_callback(m_iter, n_N_, variables_, object_func_,
                                        constraint_values_, user_pointer);
    }
    suc = true;
    if (n_IE_ + n_E_ > 0) {
...
//////////////////////////////////////////////////////////////////////////
void HLBFGS::my_eval_funcgrad(double &f, double *guess_sigma,
                              void *user_pointer) {
  double mf = 0;
  ...
  if (eval_constraints_callback && n_E_ + n_IE_ > 0) {
    ...
    eval_constraints_callback(n_E_, n_IE_, variables_, constraint_values_,
                              constraint_jacobian_, user_pointer);
...
#pragma omp parallel for reduction(+ : con_sum)
      ...
      *guess_sigma =
          std::max(1.0e-6, std::min(10.0, 2 * std::fabs(mf) / con_sum));
      return;
    }
...
#pragma omp parallel for reduction(+ : mf)
      ...
      for (ptrdiff_t i = 0; i < (ptrdiff_t)n_E_; i++) {
        for (j = 0; j < constraint_jacobian_[i].size(); j++) {
          gradient_[constraint_jacobian_[i][j].first] +=
              (-lambda[i] + dynamic_sigma[i] * d[i]) *
              constraint_jacobian_[i][j].second;
        }
      }
#ifdef USE_OPENMP
#pragma omp parallel for reduction(+ : mf)
...
#pragma omp parallel for reduction(+ : mf)
      ...
      for (ptrdiff_t i = 0; i < (ptrdiff_t)n_E_; i++) {
        for (j = 0; j < constraint_jacobian_[i].size(); j++) {
          gradient_[constraint_jacobian_[i][j].first] +=
              dynamic_sigma[i] * d[i] * constraint_jacobian_[i][j].second;
        }
      }
#ifdef USE_OPENMP
#pragma omp parallel for reduction(+ : mf)
      ...
      for (ptrdiff_t i = n_E_; i < (ptrdiff_t)(n_E_ + n_IE_); i++) {
        d[i] = std::max(0.0,
                        -constraint_values_[i] + lambda[i] / dynamic_sigma[i]);
        mf += 0.5 * dynamic_sigma[i] * d[i] * d[i];
      }
      for (ptrdiff_t i = n_E_; i < (ptrdiff_t)(n_E_ + n_IE_); i++) {
...
double HLBFGS::max_absolute_value() {
  ...
  if (info[14] == 0) {
    ...
    for (size_t i = n_E_; i < n_E_ + n_IE_; i++) {
      d_max = std::max(d_max, std::fabs(std::max(-constraint_values_[i],
                                                 -lambda[i] / sigma)));
    }
  }
  return d_max;
...
void HLBFGS::update_q_by_inverse_hessian() {
  ...
  double *prev_q_first_stage =
      prev_q_first_stage_vec.empty() ? 0 : &prev_q_first_stage_vec[0];
  double *prev_q_update = prev_q_update_vec.empty() ? 0 : &prev_q_update_vec[0];
  double scale = 0.0;
  double cg_dginit = 0;
  size_t bound = 0;
  char task1 = 'N';
  ...
  if (info[10] == 0) {
    if (info[7] == 1) {
      ICFS_INFO &l_info = m_hessian.get_icfs_info();
      dstrsol_(&n_N_, l_info.get_l(), l_info.get_ldiag(), l_info.get_lcol_ptr(),
               l_info.get_lrow_ind(), q, &task1);
      dstrsol_(&n_N_, l_info.get_l(), l_info.get_ldiag(), l_info.get_lcol_ptr(),
               l_info.get_lrow_ind(), q, &task2);
    } else {
