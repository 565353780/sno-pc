...
void OctTree::buildTree() {
  ...
  for (int i = 0; i < nodeArr.size(); ++i) {
    OctNode *nodei = nodeArr[i];
    if (!nodei->isleaf || nodei->inPointId.size() > 0 ||
        nodei->depth >= MAXDEPTH) {
      continue;
    }
    ...
    pd = round(pd / 8.0);
    if (nodei->depth < pd &&
        dists[0] < 3 * DEPLEN[nodei->depth] * DEPLEN[nodei->depth]) {
      nodei->creat_childNode(nodeArr);
    }
  }
...
void OctTree::set_gamma(double gam) { gamma = gam; }
...
void OctTree::get_outNode() {
  ...
  for (int i = 0; i < nopNode.size(); ++i) {
    ...
    Point o = node->center;
    if (o.x < pxMin || o.x > pxMax || o.y < pyMin || o.y > pyMax ||
        o.z < pzMin || o.z > pzMax) {
      outNode.push_back(i);
    }
  }
  ...
  while (!sta.empty()) {
    ...
    if (node->isleaf) {
      for (int i = 0; i < node->inPointId.size(); ++i) {
        ...
        double dis =
            pow(p.x - np.x, 2) + pow(p.y - np.y, 2) + pow(p.z - np.z, 2);
        if (pais.size() < k) {
      ...
      for (int i = 0; i < 8; ++i) {
        ...
        double bound[6] = {oi.x - d, oi.x + d, oi.y - d,
                           oi.y + d, oi.z - d, oi.z + d};
...
void OctTree::find_around() {
  for (int i = 0; i < leafNodeArr.size(); ++i) {
    ...
    double bound[6] = {oi.x - 3 * d, oi.x + 3 * d, oi.y - 3 * d,
                       oi.y + 3 * d, oi.z - 3 * d, oi.z + 3 * d};
    ...
    while (!sta.empty()) {
      ...
      if (nodej->isleaf) {
        ...
        for (int k = 0; k < 8; ++k) {
          ...
          double boundk[6] = {ok.x - 3 * dk, ok.x + 3 * dk, ok.y - 3 * dk,
                              ok.y + 3 * dk, ok.z - 3 * dk, ok.z + 3 * dk};
...
void OctTree::find_Ngbr() {
  ...
  /*for (int i = 0; i < points.size(); ++i) {
          ...
          double bound[6] = { p.x - d, p.x + d, p.y - d, p.y + d, p.z - d, p.z +
  d }; std::stack<int> sta; sta.push(0); while (!sta.empty()) { int nid =
  sta.top(); sta.pop(); OctNode *node = nodeArr[nid]; if (node->isleaf) { if
  (node->depth >= pdep[i] && isNgbr(p, node->center, node->depth)) {
...
#pragma omp critical
  ...
  for (int i = 0; i < tripletList_B[0].size(); ++i) {
    ...
    tripletList_Bw[0].push_back(
        Td(row, col, tripletList_B[0][i].value() / wds[col]));
    tripletList_Bw[1].push_back(
        Td(row, col, tripletList_B[1][i].value() / wds[col]));
    tripletList_Bw[2].push_back(
        Td(row, col, tripletList_B[2][i].value() / wds[col]));
  }
...
const bool OctTree::calc_A() {
  ...
  if (false) {
    if (!A.isApprox(A.transpose())) {
      std::cerr << "[ERROR][OctTree::calc_A]" << std::endl;
      std::cerr << "\t check symmetric for A failed!" << std::endl;
      exit(0);
      return false;
    }
  }
...
void OctTree::find_near_points() {
  ...
  for (int i = 0; i < nopNode.size(); ++i) {
    ...
    for (int k = 0; k < k_n; ++k) {
      ...
      Point p = points[pid];
      double norm =
          sqrt(pow(o.x - p.x, 2) + pow(o.y - p.y, 2) + pow(o.z - p.z, 2));
...
void func_callback_w_uv(const size_t N, const std::vector<double> &x, double &f,
                        std::vector<double> &g, void *user_supply) {
...
void OctTree::energy_evaluation_w_print_p(const std::vector<double> &x,
                                          std::vector<double> &p) {
  int N = points.size();
